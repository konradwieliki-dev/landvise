...
export default function Services() {
  ...
  const handleReportDownload = async (e: React.FormEvent) => {
    ...
    try {
      await fetch("/api/send-lead", {
        ...
        body: JSON.stringify({
          type: "raport-download",
          email: reportEmail,
          name: "",
          message: "Pobranie przykładowego raportu Poziom 1",
        }),
      });
    } catch {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="flex flex-col">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 text-[#2d6a4f] hover:text-[#40916c] text-xs font-medium mt-3 transition-colors cursor-pointer"
              >
      ...
      {/* Modal - email przed pobraniem raportu */}
      {showModal && (
        <div
        ...
        >
          ...
          >
            ...
            <h3 className="text-lg font-bold text-[#1c1917] mb-2">
              Podaj swój adres e-mail, aby pobrać Raport
            </h3>
            <p className="text-sm text-neutral-500 mb-6">
              Przykładowy raport Wstępnej Identyfikacji Kluczowych Ryzyk (Poziom 1)
            </p>

            <form onSubmit={handleReportDownload}>
              <div className="mb-4">
                <label htmlFor="report-email" className="block text-sm font-medium text-neutral-700 mb-1.5">
                  Adres e-mail
                </label>
                <input
                  type="email"
              ...
              <button
                type="submit"
                disabled={sending}
                className="w-full bg-[#2d6a4f] hover:bg-[#40916c] text-white font-semibold py-3.5 rounded-lg transition-all duration-200 disabled:opacity-50 text-sm"
              >
                {sending ? "Pobieranie..." : "Pobierz raport"}
              </button>
            </form>
          </div>
        </div>
